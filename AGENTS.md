...
- All verbs accept repeated `--home PATH --home-id ID` pairs matched positionally; unbalanced flags exit 2. `watch` fans one FSEvents (or polling) subscription per home into the single db. `chart` and `follow` take optional unpaired `--home-id` filters (default: every registered home). Single-home invocations stay byte-identical. Flags only; one process owns the db.
