...
- Tags are `vMAJOR.MINOR.PATCH` matching `plugin.json` `version`. release-please owns the version PR, tag, and changelog; `.github/workflows/release.yml` publishes darwin-arm64 and linux-amd64 archives. Do not hand-create release tags. `README.md` owns the contributor release path.
