...
const (
	...
	defaultWatchDueRetry        = 250 * time.Millisecond
...
func acquireWatchHomes(
	...
	for i, home := range homes {
		...
		if err != nil {
			retryDelay := retry
			if !home.standbyUntil.IsZero() && !home.standbyUntil.After(acquireNow) {
				retryDelay = min(retry, defaultWatchDueRetry)
			}
			home.retryAt = acquireNow.Add(retryDelay)
			home.mu.Unlock()
			failures = append(failures, err)
			continue
		}
...
func nextWatchAttempt(homes []*leasedHome, retry time.Duration) time.Duration {
	...
	for _, home := range homes {
		...
		hasRetryAt := !retryAt.IsZero()
		if retryAt.IsZero() {
			retryAt = now.Add(retry)
		}
		attemptAt := retryAt
		if !standbyUntil.IsZero() && (!hasRetryAt || standbyUntil.After(now)) && standbyUntil.Before(attemptAt) {
